...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    if [[ "$distro" == "arch" || "$id_like" == "arch" ]]; then
       ...
       sudo pacman -S --needed unace unrar zip unzip p7zip sharutils uudeview arj locate  cabextract file-roller dtc xz python-pip brotli lz4 gawk libmpack aria2
       #aur=rar
    else
